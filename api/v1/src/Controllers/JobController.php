...
class JobController
{
	...
	public function shortListedFulltime($request, $response, $args)
	{
		...
		try {
			...
			$searchKeys = str_replace(array('[', ']', '"'), "", $this->jobHelper->sanitize($postData['searchKeys']));
	...
	public function updateJob($request, $response, $args)
	{
		...
		try {
			...
			$jobPostDel = $this->job->deleteFrom('i_job_object', $condition);

			if ($jobGroupDel && $jobPostDel && $jobPostDel) {
